...
class mongo():    
	...
	self.mongo = pymongo.MongoClient('192.168.55.95', 27017)  ### Host for mongodb must be changed if the script will run localy leave it as localhost , if it will run remotely make sure that your IP is added to the EC2 security group or change it localhost if you upload it to the controller
	self.db = self.mongo['aiamhpdpintegration']
	self.collection = self.db['ticketAnalysis']
